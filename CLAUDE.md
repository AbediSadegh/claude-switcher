...
**`claude://` deep links bypass the wrapper functions.** Browsers hand them to `xdg-open`, which runs whatever `.desktop` file owns `x-scheme-handler/claude` — by default the Desktop package's own entry, which launches the binary with no `--user-data-dir`, so Electron's per-user-data-dir single-instance lock routes the link to the default profile. `claude-profile url-handler install` writes a generated script (`$CLAUDE_SWITCHER_HOME/url-handler`, which re-sources this plugin under `zsh -f` with `CLAUDE_SWITCHER_HOME` and `CLAUDE_DESKTOP_BIN` baked in) plus a `.desktop` entry, and makes it the default via `xdg-mime`. `_claude_switcher_url_profile` picks the target: the persisted profile if its Desktop instance is running (detected via the Chromium `SingletonLock` symlink's PID), else the sole running instance, else the persisted profile.
...
- Internal helpers are prefixed `_claude_switcher_*` and not meant to be called directly by users; user-facing commands (`claude-profile`, `claudeon`, `claudeoff`, `claude`, `claude-desktop`, `claude_ps1`) are unprefixed.
...
- `-h`/`--help` is implemented per-command (see `claude-profile`, `claudeon`, `claudeoff`) rather than centrally; `claude` and `claude-desktop` deliberately do *not* intercept `--help` so it passes through to the real CLI's own help.
